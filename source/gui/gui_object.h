...
class gui_object {
    ...
    gui_object() :
        ...
        font_sz(0), z_order(0), bind_screen(0), center_text(true), limit_text(0),tx(0),
        sound_played(false)
    {
    ...
    int hit_test(SDL_Event* e, int current_z)
    {
        int x = e->motion.x;
        int y = e->motion.y;
        ...
        if ((x > s_x && x < (s_x+s_w) && y > s_y && y < (s_y+s_h)))
        {
            switch(obj_type)
            {
                ...
                if (obj_state == B_OVER && !sound_played)
                {
                    sound_played = true;
                    thegame->snd->play_sound("sound/menu-sound.wav");
                }
                break;
