...
//                error!!!
    ...
    void draw(station_list* current_list,icy* ic,favorites* favs)
    {
        ...
        if (get_status() == BUFFERING) draw_info(vars.search_var("$LANG_TXT_BUFFERING"));
        if (get_status() == CONNECTING) draw_info(vars.search_var("$LANG_TXT_CONNECTING"));
    ...
    void draw_info(char* txt)
    {
        ...
        if (get_status() == BUFFERING || get_status() == CONNECTING)
        {
